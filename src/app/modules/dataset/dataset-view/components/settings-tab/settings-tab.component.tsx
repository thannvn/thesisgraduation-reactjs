...
const options = [
  { title: 'Tổng quát' },
  //Comment because upload feature is not support anymore
  // { title: 'Ảnh nền' },
  // { title: 'Thumbnail' },
];
...
export default function SettingsTab(props: SettingsTabsProps) {
  ...
  const { register, handleSubmit, errors, watch, reset } = useForm({
    defaultValues: changeDefaultValues,
  });

  const onSubmit = async (data: DefaultValuesForm) => {
    const result = await DatasetAPI.updateTitleAndSubtitle(
      datasetValues.dataset._id,
      data.title,
      data.subtitle
    );
  ...
  return (
    <div hidden={index !== value} className='t-settings-tab h-mt-32'>
        ...
        <Grid item xs={3} className='p-options'>
          {options.map((option, index) => (
            <div className='h-mt-10' key={index}>
              <button
                className={clsx(
                  { 'p-option-current': option.title === currentOption },
            ...
            <div className='b-general'>
              <form onSubmit={handleSubmit(onSubmit)}>
                <div className='p-input-field'>
                  <Typography className='h-mr-20 p-gray-color-typography'>
                    Tiêu đề
                  </Typography>
                  <TextField
                    variant='outlined'
                ...
                {errors.title && (
                  <Typography className='h-ml-100 p-validate-error h-mt-4 '>
                    Tiêu đề phải có 5-50 ký tự. Có thể sử dụng chữ và số và các
                    ký tự đặc biệt
                  </Typography>
                )}
                ...
                {errors.subtitle && (
                  <Typography className='h-ml-100 p-validate-error h-mt-4'>
                    Subtitle phải có 5-80 ký tự. Có thể sử dụng chữ và số và các
                    ký tự đặc biệt
                  </Typography>
                )}
                ...
                <div className='p-action h-mt-32'>
                  ...
                  <Button
                    ...
                    type='submit'
                    disabled={
                      watch('title') === datasetValues?.dataset.title &&
              ...
              {currentOption === options[1].title ? (
                ...
                <div>
                  {thumbnail === '' ? (
                    ...
                    <CropImage
                      src={thumbnail}
                      setCropper={setCropperThumbnail}
                    />
                  )}
                </div>
              )}
