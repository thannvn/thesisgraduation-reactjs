...
> {
  ...
  render() {
    return <DataSetListTemplate self={this} />;
  }
  ...
  handleChangeLike = async (
    ...
    const tempTopDatasets = this.setArrayLikeOrUnlike(
      this.state.datasetValuesList.topDatasets,
      datasetId,
      isLike,
      userId
    );
    const tempNewDatasets = this.setArrayLikeOrUnlike(
      this.state.datasetValuesList.newDatasets,
      datasetId,
      isLike,
      userId
    );
    const tempRecommend = this.setArrayLikeOrUnlike(
      this.state.datasetValuesList.recommend,
      datasetId,
      isLike,
      userId
    );
    tempTagsDatasets.forEach(
      (item) =>
        (item.datasets = this.setArrayLikeOrUnlike(
          item.datasets || [],
          datasetId,
          isLike,
          userId
  ...
  private setArrayLikeOrUnlike = (
    targetArray: Array<DatasetValues>,
    datasetId: string,
    isLike: boolean,
    userId: string
  ): Array<DatasetValues> => {
    return targetArray?.map((item) => {
      ...
      const newDataset: Dataset = {
        ...item.dataset,
        countLike: isLike
          ? item.dataset.countLike + 1
          : item.dataset.countLike - 1,
        like: !isLike
          ? item.dataset.like.filter((item) => item !== userId)
          : [],
      };
